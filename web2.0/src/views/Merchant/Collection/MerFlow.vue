  ...
  <div id="merFlow">
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">流水记录</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
                ...
                <div class="table-responsive mailbox-messages">
                  <b-table id="my-table" style="height:500px;" bordered responsive :items="serialList" :fields="serialFields">
                    <template v-slot:cell(change)="data">
                      <span v-if="data.item.type=='提现'">{{data.value}}</span>
                      <span v-if="data.item.type=='订单收款'" style="color:#007bff">{{data.value}}</span>
                      <span v-if="data.item.type=='订单手续费'" style="color:red;">{{data.value}}</span>
                    </template>
                  </b-table>
                </div>
  ...
  export default {
    ...
    data() {
      return {
        pageLimits: limits,
        serialFields: [
          { key: 'type', label: '类型' },{key:'relation_no',label:'关联订单号'}, { key: 'before', label: '变动前金额(元)', formatter: value => (value || 0) / 100 },
    ...
    methods: {
      ...
      async bindSerialList() {
        ...
        const data = await post('/collect/merchant/bill/list', this.form);
        if (data.code == 'ok') {
      ...
      /**可用余额 */
      async getBalance(){
        const data=await post('/collect/merchant/balance')
