        ...
        <div class="row">
          <div class="col-md-12">
            <blockquote class="quote-orange mt-0">
              <h5>代付统计</h5>
              <p>
                <b-form-select style="width:350px;" v-model="paySelected" placeholder="查看日期" :options="options" @input="paySearch"></b-form-select>
                <!-- <b-button size="sm" variant="primary" @click="paySearch()"><i class="fas fa-search">搜索</i></b-button> -->
              </p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /**代付 */
      async getPayCollect() {
        const data = await post('/pay/merchant/pay/collection')
        if (data.code == 'ok') {
      ...
      async paySearch(data) {
        ...
        if (dataArr.length > 0) {
          ...
          const data = await post('/pay/merchant/pay/collection', form)
          if (data.code == 'ok') {
      ...
      /**拼多多 */
      async getPddCollect() {
        const data = await post('/collect/merchant/pdd/collection')
        if (data.code == 'ok') {
      ...
      async pddSearch(data) {
        ...
        if (dataArr.length > 0) {
          ...
          const data = await post('/collect/merchant/pdd/collection', form)
          if (data.code == 'ok') {
